...
void WebXController::stop() {
    ...
    this->_clientRegistry.disconnectAll();
...
void WebXController::run(bool testing) {
    ...
    while (this->_state != WebXControllerState::Stopped) {
        ...
        if (this->_state != WebXControllerState::Stopped) {
            ...
            std::chrono::duration<double, std::milli> timeSinceMouseRefreshMs = start - lastMouseRefreshTime;
            lastTime = start;
            ...
            // If cursor has changed, send cursorlayout to clients
            if (this->_cursorDirty) {
                // Dispatch display event to connectors
                this->notifyCursorChanged(mouse);
            }
            ...
            if (timeSinceMouseRefreshMs.count() > MOUSE_REFRESH_DELAY_MS) {
                mouse->updatePosition();
                if (mouse->isDirty()) {
                    this->notifyMousePositionChanged(mouse);
...
void WebXController::onClientMouseInstruction(WebXDisplay * display, const std::shared_ptr<WebXMouseInstruction> & mouseInstruction, const std::shared_ptr<WebXClient> & client) {
    ...
    if (previousMousePosition.x() != mouseInstruction->x || previousMousePosition.y() != mouseInstruction->y) {
        ...
        std::shared_ptr<WebXMouseCursor> cursor = mouseState->getCursor();
        uint32_t cursorId = cursor != nullptr ? cursor->getId() : 0;
    
        // Send message with position to all clients (other that the one sending the instruction)
        this->sendMessage(std::make_shared<WebXMouseMessage>(clientIndexMask, mouseInstruction->x, mouseInstruction->y, cursorId));

        // Send message with just cursor Id to the client sending the instruction
        if (cursor != nullptr) {
            this->sendMessage(std::make_shared<WebXMouseMessage>(client->getIndex(), -1, -1, cursorId));
        }
    }
}

void WebXController::notifyCursorChanged(WebXMouse * mouse) {
    ...
    std::shared_ptr<WebXMouseCursor> cursor = mouseState->getCursor();
    if (cursor != nullptr) {
        this->sendMessage(std::make_shared<WebXMouseMessage>(GLOBAL_CLIENT_INDEX_MASK, -1, -1, cursor->getId()));
    }
}

void WebXController::notifyMousePositionChanged(WebXMouse * mouse) {
    const WebXMouseState * mouseState = mouse->getState();

    // Send message to all clients
    std::shared_ptr<WebXMouseCursor> cursor = mouseState->getCursor();
    uint32_t cursorId = cursor != nullptr ? cursor->getId() : 0;
    this->sendMessage(std::make_shared<WebXMouseMessage>(GLOBAL_CLIENT_INDEX_MASK, mouseState->getX(), mouseState->getY(), cursorId));
}
