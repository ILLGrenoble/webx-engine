...
unsigned int getStorageRequirements(unsigned int width, unsigned int height, unsigned int format) {
  switch (format)
  {
  case XB_FMT_DXT1:
...
int main() {
    ...
    unsigned int destSize = getStorageRequirements(width, height, XB_FMT_DXT5);
    unsigned char * destData = new unsigned char[destSize];

    std::chrono::high_resolution_clock::time_point start = std::chrono::high_resolution_clock::now();

    int nIter = 10;
    for (int i = 0; i < nIter; i++) {
        squish::CompressImage(imageData, width, height, destData, squish::kDxt5 | squish::kSourceBGRA);

        if (i == 0) {
            FILE * fp = fopen("test/output/screenshotOut.s3tc", "wb");
            if (!fp) {
                return false;
            }
            fwrite(destData, destSize, 1, fp);
            fclose(fp);
        }
    }
    ...
    printf("Text completed: %d iterations in %fms\n%fms / iteration for %uKB\n", nIter, duration.count() / 1000, (duration.count() / nIter) / 1000, destSize / 1024);
